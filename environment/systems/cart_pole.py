...
class CartPole(ODESystem):
    ...
    def linearize(self):
        ...
        a = np.array(
            [
                ...
                [
                    0,
                    g * total_mass / (length * cart_mass),
                    0,
                    -b * total_mass / inertia,
                ],
            ]
        )
    ...
    def _ode(self, _, state, action):
        ...
        v_dot = (
            (
                action
                - pendulum_mass * length * (omega ** 2) * np.sin(theta)
                - b * omega * np.cos(theta)
                + 0.5 * pendulum_mass * g * length * np.sin(2 * theta)
            )
            * length
            / det
        )
