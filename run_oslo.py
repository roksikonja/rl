...
class CohenController(AbstractController):
    def __init__(self, Q: np.ndarray, R: np.ndarray, system, sigma: float, theta: float, nu: float,
                 delta: float = 0.1, time_horizon: int = 1000, A0=None, B0=None, W=None, alpha_0=None,
                 initial_state=None, initial_policy=None):
        self.Q = Q
        self.R = R
        self.system = system
        self.sigma = sigma
    ...
    def step(self, observation: np.ndarray) -> Union[np.ndarray, None]:
        if self._time >= self.time_horizon:
            warnings.warn("You have reached time horizon, the algorithm provides no more actions!")
            return None
        self._x_history = np.roll(self._x_history, 1, axis=0)
        self._x_history[0, :] = observation.T
        self.state = observation
        self._X_top_Z = self._X_top_Z + self.state.reshape(-1, 1) @ self._z.reshape(1, -1)
        # print(self._X_top_Z)
        if self._time == 0 or np.linalg.det(self.V1) > 2 * np.linalg.det(self.V0) or self._time % 500 == 0:
            logging.info('Controller update number: '.format(self.control_update_counter))
            ...
            S = self._relaxed_sdp(self.A, self.B, self.Q, self.R, self.W, self.mu, self.V1)
            self.policy = S[self.dim_state:, :self.dim_state] @ np.linalg.inv(S[:self.dim_state, :self.dim_state])
    ...
    def _relaxed_sdp(self, A: np.ndarray, B: np.ndarray, Q: np.ndarray, R: np.ndarray, W: np.ndarray, mu: float,
        ...
        constraints = [S >> 0]
        constraints += [
            S[:d, :d] >> np.block([A, B]) @ S @ np.block([A, B]).T + W -
...
if __name__ == "__main__":
    ...
    controller = CohenController(Q=Q, R=R, system=env, sigma=sigma_w, theta=theta, nu=nu, time_horizon=3000)
    ...
    plt.plot(np.flip(np.linalg.norm(controller._z_history[:, d:], axis=1)), label='action')
    plt.yscale('log')
    plt.legend()
    plt.show()
